...
class PGStrategyTracker(pgfinancebase.PGFinanceBase, pgfinancecommon.PGFinanceCommon):
    ...
    def pg_actions(self, db_session=None):
    ...
    def save(self, dirpath: str=None):
        ...
        try:
            ...
            if dirpath:
                ...
                _dirpath = self._config.parameters["config_file"]["default"]["data_loc"] if not args and args.pg_data_dir else args.pg_data_dir

            _existing_files = self.pg_deserialize_from_disk(os.path.join(_dirpath, "_processed._internal"))
            for file_name in _processed(_dirpath, _existing_files):
                _pg_filepath = os.path.join(_dirpath, file_name)
                if pgfile.is_non_zero_file(_pg_filepath):
                    print(file_name)
                    if self.save_to_db(args.pg_tablename, self.pg_deserialize_from_disk(_pg_filepath)):
                        _existing_files[file_name] = "processed"
                    else:
                        print("processed failed")
                else:
                    print(f"skipping, {_pg_filepath} is empty")

            self.pg_serialize_to_disk(_existing_files, os.path.join(_dirpath, "_processed._internal"))
...
class PGStrategyTrackerExt(PGStrategyTracker):
...
class PGStrategyTrackerSingleton(PGStrategyTracker):
    __instance = None

    @staticmethod
    def get_instance():
        if PGStrategyTrackerSingleton.__instance == None:
            PGStrategyTrackerSingleton()
        else:
            return PGStrategyTrackerSingleton.__instance

    def __init__(self, project_name: str = "pgpaasingleton", logging_enable: str = False):
        super(PGStrategyTrackerSingleton, self).__init__(project_name=project_name, logging_enable=logging_enable)
        PGStrategyTrackerSingleton.__instance = self


if __name__ == '__main__':
    test = PGStrategyTrackerExt()
    test.save()
