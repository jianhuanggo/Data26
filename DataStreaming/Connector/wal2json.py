...
class PostgresLogParse:
    ...
    def __init__(self, args):
        ...
        self.count = 0
        self.initial_filename = pgfile.get_random_filename('replication.log')
        self.filename = open(self.initial_filename, 'a')
        self.last_lsn = None
        self.time_sleep = 0
        ...
        self.list_files = []
        self.list_files.append(self.initial_filename)
            ...
            if self.msg:
                ...
                if self.count > self.lines_per_file:
                    try:
                        ...
                        if self.args.test_flg:
                            break
                        self.filename = open(pgfile.get_random_filename('replication.log'), 'a')
                        self.count = 0
                    except Exception as err:
                        raise err
    ...
    def test(self):
        with open(self.initial_filename) as f:
...
class JsonPar:
    def __init__(self):
        self.string_count = 0
        self.string_index = 0
        self.string_start_index = 0
        self.string_end_index = 0
        self.para = []

    def return_json_object(self, string_text, start_index=0):
        if len(string_text) < start_index:
            raise ("index is larger than the length of object")

        #print(string_text.decode('utf8'))
        #string_text.replace('\n', '').replace('\r', '')
        for index, item in enumerate(string_text):
            if item == '{':
                self.para.append('{')
                self.string_start_index = index
                break

        self.string_count = 0

        string_text1 = string_text[self.string_start_index + 1:]
        for index, char in enumerate(string_text1):
            #if self.string_count == 0 and char != '{':
            #    raise ("This is not a json text")
            if char == '{':
                self.para.append('{')

            if char == '}':
                self.para.pop()
                self.string_end_index = index

            if not self.para:
                break

        return string_text[self.string_start_index:self.string_end_index + 1].strip()


if __name__ == '__main__':
    #args = get_parser()
    #wat_plugin = PostgresLogParse(args)
    with open('c01fed_replication.json', 'rb') as f:
        input_text = str(f.readline())

    print(input_text)
    print(JsonPar().return_json_object(input_text))

    with open('json_out.json', 'w') as f:
        f.write(JsonPar().return_json_object(input_text))


    #print(wat_plugin.return_json_object(input_text))
    exit(0)
