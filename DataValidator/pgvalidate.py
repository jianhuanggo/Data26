...
from API.Finance.PandasDataReader import pgpandasreader
...
def create_session(object_type, subscription_level=1, logger=None):
    ...
    action = {"pgprofile": {'1': pgpandasreader.PGPandasDataReader(),
                                   '2': pgpandasreader.PGPandasDataReaderExt(),
                                   '999': pgpandasreader.PGPandasDataReaderSingleton(),
                                   },
              }

    parameter = {'proprofile':  {'1': pgpandasreader.PGPandasDataReader(),
                                       '2': pgpandasreader.PGPandasDataReaderExt(),
                                       '999': pgpandasreader.PGPandasDataReaderSingleton(),
                                      },
                 }
...
def pg_validate(object_type: str, object_name: str = None, variable_name: str ="_pg_action", subscription_level: int = 1) -> Callable[[F], F]:
