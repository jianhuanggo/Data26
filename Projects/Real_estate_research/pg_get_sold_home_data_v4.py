...
def pg_worker_v2(client_num: int, parameters=None, tasks=None,
    ...
    try:
        ...
        _action_tasks = None
        if _pg_action:
            if _pg_action[parameters['_pg_action']].get_tasks(parameters['address_file'], parameters):
                _action_tasks = _pg_action[parameters['_pg_action']]._data_inputs[parameters['name']]
            else:
