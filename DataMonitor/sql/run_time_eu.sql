select job_instance_id, job_name, priority_value, ET- ST duration from (
	...
	rank() OVER (PARTITION BY metaschemaeu.scd_job_instance.job_id ORDER BY cast(metaschemaeu.scd_job_instance.job_instance_id as integer) DESC) AS rnk 
	FROM metaschemaeu.scd_job_instance where status = 'DONE') AS anon_2 where anon_2.rnk = 1 order by 2
