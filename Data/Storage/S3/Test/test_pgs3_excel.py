...
def blocks(client_num: int, parameter, storage=None, storage_format=None):
    ...
    object_key = f"file_{str(uuid.uuid4().hex[:8])}.xlsx"
...
@pgstorage.pgstorage(storage_type='s3')
@pgstorageformat.pgstorageformat(storage_format='excel')
def get_count(bucket, storage=None, storage_format=None):
    result = []
...
class MyTest(unittest.TestCase):
    ...
    async def test_decorator_asyncio_multithreading(self):
        ...
        for _key in s3.list_keys(bucket='testba8a4bda'):
            #print(key['Key'])
            s3.delete_s3_obj(bucket_name='testba8a4bda', key=_key)
