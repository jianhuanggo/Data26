...
def pg_worker_v2(client_num: int, parameters=None, tasks=None,
    ...
    try:
        ...
        _action_tasks = None
        if _pg_action:
            _address_file = "/Users/jianhuang/opt/anaconda3/envs/Data20/rl_data/real_estate/test_v2/output_test.json"
            if _pg_action[parameters['_pg_action']].get_tasks(_address_file, {'name': "john_creek_sold_homes",
                                                                              'read_func': pgscrapyformatter.PGScrapFormatter().redfin_address_format}):
                _action_tasks = _pg_action[parameters['_pg_action']]._data_inputs["john_creek_sold_homes"]
                print(f"list of task: {_action_tasks}")
            else:
        ...
        if _pg_storage:
            ...
            for _ind, _val in _pg_action[parameters['_pg_action']].data.items():
                if _ind.startswith("house_"):
                    print(_pg_action[parameters['_pg_action']].data[_ind])

                    _pg_db['db_mysql_default'].pg_save("re_house",
                                      _pg_action[parameters['_pg_action']].data[_ind],
                                      "/Users/jianhuang/opt/anaconda3/envs/Data20/rl_data/real_estate/test_v2/exception_data_4_db.txt")

                elif _ind.startswith("housedetail_"):
                    _pg_db['db_mysql_default'].pg_save("re_house_detail",
                                      _pg_action[parameters['_pg_action']].data[_ind],
                                      "/Users/jianhuang/opt/anaconda3/envs/Data20/rl_data/real_estate/test_v2/exception_data_4_db.txt")
...
async def get_data(_pg_action=None):
    _process_object_map = {'_pg_trigger': {},
                           '_pg_action': {'redfin': pgrealestate.api_real_estate},
                           '_pg_storageformat': {},
                           '_pg_storage': {},
                           #'_pg_db': {'mysql': "ok"}
                           }

    parameters = {'_pg_trigger': (1,),
                  '_pg_action': ("redfin", "", "_pg_action", 999),
                  '_pg_storageformat': (),
                  '_pg_storage': (),
                  #'_pg_db': {'mysql': "ok"}
                  }
    ...
    _parameters = {}
    _news_proc = pg_worker
    total_client_number = 1

    for _element, _detail in _process_object_map.items():
        if _detail:
            for _object_type, _object in _detail.items():
                _news_proc = _object(object_type=_object_type, object_name="", variable_name=_element, subscription_level=999)(_news_proc)
...
async def get_data(_pg_action=None):
    _process_object_map = {'_pg_trigger': {},
                           '_pg_action': pgrealestate.api_real_estate,
                           '_pg_storageformat': {},
                           '_pg_storage': {},
    ...
    total_client_number = 1
    news_run_tasks = pgprocessing.pg_asyncio()(pgprocessing.pg_multithreading(num_workers=total_client_number)(pgprocessing.pg_run_tasks))
    ret = await news_run_tasks(func=_news_proc, num_client=total_client_number, parameter=_func_parameters)
    print(ret)
