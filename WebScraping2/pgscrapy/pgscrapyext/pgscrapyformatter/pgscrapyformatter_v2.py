...
class PGWebScrapingFormatter(pgscrapyformatterbase.PGScrapingFormatterBase, pgscrapyformattercommon.PGScrapingFormatterCommon):
    ...
    def pg_data_acquisition(self, filepath: str, custom_validate=None, top_data_num: int = 1):
        """
        mode for beautifulsoup: html.parser, html5lib
        ...
        def _find_num_record(pg_data_content: Union[bs4.element.Tag, bs4.element.ResultSet],
                             expected_item_num: int = None, top_data_num: int = 1):
            _cache = collections.defaultdict(tuple)

            print(len(pg_data_content))
            for index, item in enumerate(pg_data_content):
                #print(len(item))
                print(f"{index} --------->, {item}, {len(item)}, {type(item)}")
            ...
            print(pg_data_content[598])
            _extract_stats(pg_data_content[598], 0)
            print(_cache)
        ...
        with open(filepath) as file:
            ...
            return _find_num_record(_entire_finding)
            #return _entire_finding, _find_num_record(_entire_finding)

    def pg_extract_data(self, pg_data, custom_parser=pg_default_formatter):
    ...
    def test_case(self, dirpath: str, filename: str):
        ...
        if Path.exists:
            ...
            _pg_data_content = self.pg_data_acquisition(os.path.join(dirpath, filename))
            ...
            self.pg_extract_data(_pg_data_content)
...
if __name__ == '__main__':
    ...
    #test.test_case(_pg_file_dir, "test31.html")
    test.test_case(_pg_file_dir, "test42.html")
